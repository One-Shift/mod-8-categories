...
class c8_category
{
    ...
    public function update()
    {
        ...
        if ($db->query(sprintf(
            "UPDATE %s_8_categories SET parent_id = %d, category_section = '%s', code = '%s', sort = %d, date = '%s', user_id = %d, published = %d WHERE id = %d",
            $cfg->db->prefix,
            $this->parent_id,
            $this->category_section,
            $this->code,
            ...
            foreach ($cfg->lg as $index => $l) {
                if ($l[0]) {
                    if (!$db->query(sprintf(
                        "UPDATE %s_8_categories_lang SET title = '%s', text = '%s', `meta-keywords` = '%s', `meta-description` = '%s' WHERE category_id = %d AND lang_id = %d",
                        $cfg->db->prefix,
    ...
    public function delete()
    {
        ...
        if ($trash->insert()) {
            if ($db->query(sprintf(
                ...
                if ($db->query(sprintf("DELETE FROM %s_8_categories_rel WHERE category_id = %d", $cfg->db->prefix, $this->id))) {
                    return TRUE;
                }
    ...
    public function returnOneCategory()
    {
        ...
        $source = $db->query(sprintf(
            "SELECT bc.*, bcl.`title`, bcl.`text`, bcl.`meta-keywords`, bcl.`meta-description`
				...
				WHERE bc.id = %d and bcl.lang_id = %d",
            $cfg->db->prefix,
            $cfg->db->prefix,
            $this->id,
            $this->lang_id
    ...
    public function returnNrChildsCategory()
    {
        ...
        $source = $db->query(sprintf(
            "SELECT COUNT(id) as 'nr_sub_cats' FROM %s_8_categories WHERE parent_id = %d",
            $cfg->db->prefix,
            $this->id
        ));
    ...
    public function returnAllMainCategories()
    {
        ...
        $source = $db->query(sprintf(
            "SELECT bc.*, bcl.title, bcl.text, bcl.`meta-keywords`, bcl.`meta-description`, (SELECT COUNT(id) FROM %s_8_categories WHERE parent_id = bc.id) AS 'nr_sub_cats'
				FROM %s_8_categories bc
					INNER JOIN %s_8_categories_lang AS bcl on bcl.category_id = bc.id
				WHERE bc.parent_id = -1 AND bcl.lang_id = %d
    ...
    public function returnChildCategories()
    {
        ...
        $query = sprintf(
            "SELECT bc.*, bcl.title, bcl.text, bcl.`meta-keywords`, bcl.`meta-description`, (SELECT COUNT(id) FROM %s_8_categories WHERE parent_id = bc.id) AS 'nr_sub_cats'
				FROM %s_8_categories bc
					INNER JOIN %s_8_categories_lang AS bcl on bcl.category_id = bc.id
				WHERE bc.parent_id = %d AND bcl.lang_id = %d
				ORDER BY bc.sort ASC, bc.category_section ASC, bcl.title ASC",
            $cfg->db->prefix,
            $cfg->db->prefix,
            $cfg->db->prefix,
    ...
    public static function checkRel($id = null)
    {
        ...
        if (!is_null($id) && $id != 0) {
            ...
            $source = $db->query(sprintf(
                "SELECT * FROM %s_8_categories_rel WHERE category_id = %d",
                $cfg->db->prefix,
                $id
            ));
    ...
    /** [Return all related categories by given object_id and defining OR not, what module is being used]
